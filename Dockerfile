...
RUN \
  ...
  apt update && \
  apt install -y build-essential make g++ gcc p7zip unzip && \
...
RUN \
  ...
  apt update && \
  apt install -y findutils openssl python3 transmission-daemon transmission-cli cron ca-certificates && \
  apt clean

COPY --from=builder /transmissionic /transmissionic
COPY --from=builder /transmission-web-control /transmission-web-control
COPY --from=builder /kettu /kettu
COPY --from=builder /flood-for-transmission /flood-for-transmission
COPY --from=builder /combustion-release /combustion-release
